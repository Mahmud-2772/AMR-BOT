...
module.exports = {
		...
		if (event.logMessageType == "log:unsubscribe")
			return async function () {
				...
				if (!threadData.settings.sendLeaveMessage)
					return;
				const { leftParticipantFbId } = event.;
				if (leftParticipantFbId == api.getCurrentUserID নিছে",
		...
		if (event.logMessageType == "log:unsubscribe")
			return async function () {
				...
				if (!threadData.settings.sendLeaveMessage)
					return;
				const { leftParticipantFbId } = event.;
				if (leftParticipantFbId == api.getCurleave নিছে",
			leaveType2: "ঘাড় ধাক্যা দেয়া হলো",
			defaultLeaveMessage: "{userName} {type}"
		}
	},
		...
		if (event.logMessageType == "log:unsubscribe")
			return async function () {
				...
				if (!threadData.settings.sendLeaveMessage)
					return;
				const { leftParticipantFbId } = event.;
				if (leftParticipantFbId == api.getCurr নিছে",
			leaveType2: "ঘাড় ধাক্যা দেয়া হলো",
			defaultLeaveMessage: "{userName} {type}"
		}
	},
		...
		if (event.logMessageType == "log:unsubscribe")
			return async function () {
				...
				let { leaveMessage = getLang("defaultLeaveMessage") } = threadData.data;
				 form = {
					mentions: leaveMessage.match(/\{userName Tag\}/g) ? [{
						tag: userName,
						id: 
					}] : null
				};
				...
				if (leftParticipantFbId == api.getC",
	...
	onStart: async ({ threadsData, message, event, api, usersData, getLang }) => {
		if (event.loname of the user who left the group
				// {type}       : type of the message (leave)
				// {boxName}    : name of the box
				// {threadName} : name of the box
				// {time}       : time
...
const form = {
				...
				if (threadData.data.leaveAttachment) {
					cfiles = threadData.data.leaveAttachment;
...
};files = threadData.data.leaveAttachment;
					const attachments = files.reduce((acc, file) => {
						acc.push(drive.getFile(file, "stream"));
						return acc;
					}, []);
				...
				if (threadData.data.leaveAttachment) {
					const files = threadData.data.leaveAttasageType == "log:unsubscribe")
			return async function () {
				const { threadID } = event;
				const threadData = await threadsData.get(threadID);
				if (!threadData.settings.sendLeaveMessage)
	...
	onStart: async ({ threadsData, message, event, api, usersData, getLang }) => {
		if (event.logMessageType == "log:unsubscribe")
			return async function () {
				...
				if (leaveMessage.includes("{userNameTag}")) userName = await usersData.getName(leftParticipantFbId);
